...
from flychess.widgets import createImage, dock_panel_tab, mainwindow, gtk_close
from flychess.widgets.pydock import SOUTH
    ...
    def __init__(self, name, label):
        ...
        if getattr(sys, "frozen", False) and not MSYS2:
            ...
            importer = zipimport.zipimporter(
                zip_path + "/flychess/perspectives/%s" % name
            )
            postfix = "Panel.pyc"
            ...
            self.sidePanels = [
                importlib.import_module(f"flychess.perspectives.{name}.{f}")
                for f in files
            ]
        ...
        if os.path.isfile(self.dockLocation):
            try:
                ...
                if (
                    ...
                    msg_dia.set_markup(
                        _(
                            "<b><big>flychess was unable to load your panel settings</big></b>"
                        )
                    )
