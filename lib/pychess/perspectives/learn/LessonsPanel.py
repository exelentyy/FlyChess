...
from flychess.System.prefix import addDataPrefix
from flychess.Utils.const import WHITE, BLACK, LOCAL, WAITING_TO_START, HINT, LESSON
...
from flychess.perspectives.learn.generateLessonsSidepanel import generateLessonsSidepanel
from flychess.perspectives.learn import lessons_solving_progress
from flychess.perspectives.learn.PuzzlesPanel import start_puzzle_game
...
for elem in sorted(os.listdir(path=addDataPrefix("learn/lessons/"))):
    if elem.startswith("lichess_study") and elem.endswith(".pgn"):
        ...
        LESSONS.append((elem, elem.replace("-", " ").capitalize(), "flychess.org"))
...
def start_lesson_game(gamemodel, filename, chessfile, records, index, rec):
    ...
    w_name = player_name if color == WHITE else "flychess"
    b_name = "flychess" if color == WHITE else player_name
