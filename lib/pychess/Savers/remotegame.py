...
from flychess.Utils.lutils.lmove import parseAny, toSAN
...
class InternetGameInterface:
    ...
    def __init__(self):
        ...
        self.userAgent = "flychess %s" % VERSION
...
class InternetGameChessgames(InternetGameInterface):
    ...
    def download_game(self):
        ...
        url = "http://www.chessgames.com/pgn/flychess.pgn?gid=" + self.id
        if self.computer:
...
# flychess.org
class InternetGameflychess(InternetGameInterface):
    def get_identity(self):
        return "flychess.org", CAT_WS

    def assign_game(self, url):
        # Retrieve the ID of the game
        rxp = re.compile(
            r"https?:\/\/(www\.)?flychess(-variants\.herokuapp\.com|\.org)\/([a-z0-9]+)[\/\?\#]?",
            re.IGNORECASE,
        )
        m = rxp.match(url)
        if m is not None:
    ...
    async def download_game(self):
        ...
        async def coro(self):
            result = None
            ws = await websockets.connect(
                "wss://www.flychess.org/wsr",
                origin="https://www.flychess.org",
                ping_interval=None,
            )
            try:
