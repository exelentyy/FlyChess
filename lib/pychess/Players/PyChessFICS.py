...
import flychess
from flychess.Players.flychess import flychess
from flychess.System.repeat import repeat_sleep
from flychess.System import fident
from flychess.System.Log import log
from flychess.Utils.const import (
    WHITE,
    PAUSE_OFFER,
    DRAW_OFFER,
    BLACK,
...
from flychess.Utils.lutils.LBoard import LBoard
from flychess.Utils.lutils.lmove import determineAlgebraicNotation, toLAN, parseSAN
from flychess.Utils.lutils import lsearch
from flychess.Utils.repr import reprResult_long, reprReason_long
from flychess.ic.FICSConnection import FICSMainConnection


class flychessFICS(flychess):
    def __init__(self, password, from_address, to_address):
        flychess.__init__(self)

        self.ports = (23, 5000)
        if not password:
            self.username = "guest"
        else:
            self.username = "flychess"
        self.owner = "Lobais"
        self.password = password
        self.from_address = "The flychess Bot <%s>" % from_address
        ...
        # The amount of random challenges, that flychess sends with each seek
        self.challenges = 10
        # enableEGTB()
    ...
    def makeReady(self):
        ...
        self.connection.fm.setFingerNote(
            1,
            "flychess is the chess engine bundled with the flychess %s " % flychess.VERSION
        ...
        self.connection.fm.setFingerNote(
            2,
            "flychess is 100% Python code and is released under the terms of "
        ...
        self.connection.fm.setFingerNote(
            3,
            "flychess runs on an elderly AMD Sempron(tm) Processor 3200+, 512 "
        ...
        self.connection.fm.setFingerNote(
            4,
            "flychess uses a small 500 KB openingbook based solely on Kasparov "
        ...
        self.connection.fm.setFingerNote(
            5,
            "flychess will allow any pause/resume and adjourn wishes, but will "
            + "deny takebacks. Draw, abort and switch offers are accepted, "
            + "if they are found to be an advance. Flag is auto called, but "
            + "flychess never resigns. We don't want you to forget your basic "
            + "mating skills.",
        )
    ...
    def __usage(self):
        return (
            "|| flychess bot help file || "
            + "# help 'Displays this help file' "
            + "# sudo <password> <command> 'Lets flychess execute the given command' "
            + "# sendlog 'Makes flychess send you its current log'"
        )
    ...
    def __go(self):
        ...
        #     lambda worker: flychess._flychess__go(self, worker))
...
if __name__ == "__main__":
    if len(sys.argv) == 5 and sys.argv[1] == "fics":
        flychess_fics = flychessFICS(*sys.argv[2:])
    else:
        print("Unknown argument(s):", repr(sys.argv))
        sys.exit(0)

    flychess_fics.makeReady()
    flychess_fics.run()
