...
from flychess.System.command import Command
from flychess.System.SubProcess import SubProcess
from flychess.System.prefix import (
    addUserConfigPrefix,
    getDataPrefix,
    getEngineDataPrefix,
)
from flychess.Players.Player import PlayerIsDead
from flychess.Utils import createStoryTextAppEvent
...
from flychess.Players.engineList import PYTHONBIN, VM_LIST, ENGINES_LIST
...
class EngineDiscoverer(GObject.GObject):
    ...
    def __findRunData(self, engine):
        ...
        For the flychess engine, special handling is taken, and we search
        ...
        if engine.get("vm_name") is not None:
            ...
            if engine["name"] == "flychess.py" and not getattr(sys, "frozen", False):
                path = join(abspath(dirname(__file__)), "flychess.py")
    ...
    def __clean(self, rundata, engine):
        ...
        if refeng is None:
            log.warning(
                "Engine '%s' is not in flychess predefined known engines list"
                % engine.get("name")
            )
            engine["recheck"] = True
        else:
