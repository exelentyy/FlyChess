...
# from flychess.ic import TimeSeal
from flychess.Savers import pgn
from flychess.System import conf
from flychess.System import uistuff
from flychess.System import download_file_async
...
class ExternalsDialog:
    def __init__(self):
        ...
        label = Gtk.Label(
            _(
                "Some flychess features require further permission in order to download external components"
            )
        )
        ...
        link = "https://github.com/flychess/scoutfish"
        ...
        box.add(check_button)
        link = "https://github.com/flychess/chess_db"
        link_button = Gtk.LinkButton(link, link)
        box.add(link_button)
        vbox.pack_start(box, False, False, 0)
    ...
    def on_close_clicked(self, button):
        async def coro():
            ...
            if pgn.scoutfish_path is None and conf.get("download_scoutfish"):
                binary = (
                    "https://github.com/flychess/scoutfish/releases/download/20170627/%s"
                    % pgn.scoutfish
                )
            ...
            if pgn.chess_db_path is None and conf.get("download_chess_db"):
                binary = (
                    "https://github.com/flychess/chess_db/releases/download/20170627/%s"
                    % pgn.parser
                )
                filename = await download_file_async(binary)
                if filename is not None:
