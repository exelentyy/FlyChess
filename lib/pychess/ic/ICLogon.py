...
from flychess.ic.FICSConnection import (
    FICSMainConnection,
...
class ICLogon:
    ...
    # https://github.com/flychess/flychess/issues/1375
    def password_set_value(self, entry, value):
        entry.set_text(str(value))

    # workaround to Can't type IP to FICS login dialog
    # https://github.com/flychess/flychess/issues/1360
    ...
    def onHelperConnectionError(self, connection, error):
        if self.helperconn is not None:
            ...
            text = (
                "flychess can maintain users status and games list only if it changes\n\
