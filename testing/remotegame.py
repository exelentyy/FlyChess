...
from flychess.Savers.remotegame import (
...
class RemoteGameTestCase(unittest.TestCase):
    ...
    def testLichess(self):
        links = [
            ...
            (
                "https://lichess.org/broadcast/2019-flychess-round-1/flychess1",
                False,
    ...
    def testGameknot(self):
        links = [
            ...
            (
                ...
                False,
            ),  # Random puzzle but rejected by flychess (wrong format)
    ...
    def test2700chess(self):
        links = [
            ...
            (
                "https://2700chess.COM/games/flychess-r1.1-paris-fra-2019-12-25",
                False,
            ),  # Not a game (wrong ID)
    ...
    def testflychess(self):
        links = [
            ("http://flychess.org/DGN5Ps2k#tag", True),  # Crazyhouse
            ("http://flychess-variants.herokuapp.com/uWbgRNfw", True),  # Chess
            ("http://flychess.org/4XTiOuKB", True),  # Chess960
            ("http://flychess.ORG/b8aZwvoJ", True),  # Placement
            ("https://flychess.org/drtDbEhd#tag", False),  # Shogi (unsupported)
            ("https://flychess.ORG/tALxtipo", True),  # Makruk
            ("https://flychess.org/2CKjayxv?param", True),  # Cambodian
            ("https://flychess.ORG/4x0kQ8kY", True),  # Sittuyin
            ("https://flychess.org/7cqV5j2N", True),  # Seirawan
            ("http://flychess.org", False),
        ]  # Not a game (homepage)
        self.executeTest(InternetGameflychess(), links)
