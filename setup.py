...
if sys.platform == "win32":
    try:
        ...
        print("ERROR: flychess in Windows Platform requires to install PyGObject.")
...
spec = importlib.machinery.PathFinder().find_spec("flychess", ["lib"])
flychess = importlib.util.module_from_spec(spec)
spec.loader.exec_module(flychess)

VERSION = flychess.VERSION

NAME = "flychess"

DESC = "Chess client"

LONG_DESC = """flychess is a chess client for playing and analyzing chess games. It is
...
flychess has a builtin python chess engine and auto-detects most
...
and are used to provide hints and analysis. flychess also shows analysis
...
flychess has a built-in Timeseal client, so you won't lose clock time during
a game due to lag. flychess also has pre-move support, which means you can
make (or start making) a move before your opponent has made their move.

flychess has many other features including:
...
# UI
DATA_FILES += [("share/flychess/glade", glob("glade/*.glade"))]
DATA_FILES += [("share/flychess/glade", ["glade/background.jpg"])]
DATA_FILES += [("share/flychess/glade", glob("glade/*.png"))]
DATA_FILES += [("share/flychess/glade/16x16", glob("glade/16x16/*.png"))]
DATA_FILES += [("share/flychess/glade/48x48", glob("glade/48x48/*.png"))]
DATA_FILES += [("share/flychess/glade", glob("glade/*.svg"))]
DATA_FILES += [("share/flychess/flags", glob("flags/*.png"))]
DATA_FILES += [("share/flychess/boards", glob("boards/*.png"))]

# Data
DATA_FILES += [("share/mime/packages", ["flychess.xml"])]
DATA_FILES += [("share/metainfo", ["flychess.metainfo.xml"])]
DATA_FILES += [("share/applications", ["flychess.desktop"])]
DATA_FILES += [("share/icons/hicolor/scalable/apps", ["flychess.svg"])]
if sys.platform == "win32":
    DATA_FILES += [("share/flychess/sounds", glob("sounds/*.wav"))]
    DATA_FILES += [("share/flychess/engines", glob("engines/*.*"))]
else:
    DATA_FILES += [("share/flychess/sounds", glob("sounds/*.ogg"))]
DATA_FILES += [("share/icons/hicolor/24x24/apps", ["flychess.png"])]
DATA_FILES += [
    (
...
# Piece sets
DATA_FILES += [("share/flychess/pieces", glob("pieces/*.png"))]

if not isfile(os.path.abspath("learn/puzzles/mate_in_4.sqlite")):
    from flychess.Savers.pgn import PGNFile
    from flychess.System.protoopen import protoopen
...
DATA_FILES += [("share/flychess/learn/puzzles", glob("learn/puzzles/*.olv"))]
DATA_FILES += [("share/flychess/learn/puzzles", glob("learn/puzzles/*.pgn"))]
DATA_FILES += [("share/flychess/learn/puzzles", glob("learn/puzzles/*.sqlite"))]
DATA_FILES += [("share/flychess/learn/lessons", glob("learn/lessons/*.pgn"))]
DATA_FILES += [("share/flychess/learn/lessons", glob("learn/lessons/*.sqlite"))]
DATA_FILES += [("share/flychess/learn/lectures", glob("learn/lectures/*.txt"))]

for dir in [d for d in listdir("pieces") if isdir(os.path.join("pieces", d))]:
    DATA_FILES += [("share/flychess/pieces/" + dir, glob("pieces/" + dir + "/*.svg"))]

# Manpages
DATA_FILES += [("share/man/man1", ["manpages/flychess.1.gz"])]

# Language
pofile = "LC_MESSAGES/flychess"
if sys.platform == "win32":
...
if msi:
    ...
    # gtk3.0 .mo files
    gtk_mo = [
        f + "/LC_MESSAGES/gtk30.mo" for f in os.listdir(lang_path) if f in flychess_langs
    ]
    ...
    executables = [
        Executable(
            ...
            base=base,
            icon="flychess.ico",
            shortcut_name="flychess",
            shortcut_dir="DesktopFolder",
        ),
        Executable(
            script="lib/__main__.py", target_name="flychess-engine.exe", base=base
        ),
    ]
    ...
    perspectives = ["flychess.perspectives"]
    for persp in ("welcome", "games", "fics", "database", "learn"):
        perspectives.append("flychess.perspectives.%s" % persp)
    ...
    PACKAGES = [
        ...
        "flychess.gfx",
        "flychess.ic",
        "flychess.ic.managers",
        "flychess.Players",
        "flychess.Savers",
        "flychess.System",
        "flychess.Utils",
        "flychess.Utils.lutils",
        "flychess.Variants",
        "flychess.Database",
        "flychess.widgets",
        "flychess.widgets.pydock",
        "flychess.perspectives",
        "flychess.perspectives.welcome",
        "flychess.perspectives.games",
        "flychess.perspectives.fics",
        "flychess.perspectives.database",
        "flychess.perspectives.learn",
        "flychess.external",
    ]
...
setup(
    name=NAME,
    version=VERSION,
    author="flychess team",
    author_email="flychess-people@googlegroups.com",
    ...
    license="GPL3",
    url="https://flychess.github.io/",
    download_url="https://github.com/flychess/flychess/releases",
